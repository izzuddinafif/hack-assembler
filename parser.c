...
const MnemonicMap comp_table[] = {
    ...
    {"D|M", "1010101"}, {nullptr, nullptr}};

const MnemonicMap dest_table[] = {
    {"null", "000"}, {"M", "001"},   {"D", "010"},
    {"MD", "011"},   {"A", "100"},   {"AM", "101"},
    {"AD", "110"},   {"AMD", "111"}, {nullptr, nullptr}};

const MnemonicMap jump_table[] = {
    {"null", "000"}, {"JGT", "001"}, {"JEQ", "010"},
    {"JGE", "011"},  {"JLT", "100"}, {"JNE", "101"},
    {"JLE", "110"},  {"JMP", "111"}, {nullptr, nullptr}};

const char *lookup_mnemonic(const MnemonicMap *table,
                            const char *mnemonic_to_find) {
...
bool parser_init(Parser *parser, const char *Filename) {
  ...
  DEBUG_LOG(dbg, "opening file: %s", Filename);
  if (!file) {
    return false;
  }
...
bool advance(Parser *parser) {
  ...
  while (fgets(line_buf, sizeof line_buf, parser->inputFile)) {
    ...
    snprintf(parser->currentInstruction, sizeof parser->currentInstruction,
             "%s", line_buf);
    return true;
  }
...
void _parser_symbol_print_error(int ln, const char *invalid_symbol_ptr,
                                const char *type, const char *instruction,
                                const char *symbol, int pos) {
  int invalid_symbol_pos =
      pos + (int)(strchr(symbol, *invalid_symbol_ptr) - symbol);
  if (isdigit((unsigned char)*invalid_symbol_ptr) && invalid_symbol_pos == 1) {
    print_syntax_error(instruction, type, ln, invalid_symbol_pos,
                       "symbol can't start with digit \'%c\'",
                       *invalid_symbol_ptr);
  } else if (isspace((unsigned char)*invalid_symbol_ptr)) {
    print_syntax_error(instruction, type, ln, invalid_symbol_pos,
                       "invalid whitespace \'%c\'", *invalid_symbol_ptr);
  } else {
    print_syntax_error(instruction, type, ln, invalid_symbol_pos,
                       "invalid symbol \'%c\'", *invalid_symbol_ptr);
  }
}

void get_symbol(Parser *parser, SymbolTable *symbol_table, Error *err) {
  ...
  if (type == A_INSTRUCTION) {
    if (strlen(instruction) < 2) {
      print_syntax_error(instruction, parser->typeString, ln,
                         (int)strlen(instruction), "missing symbol after @");
    ...
    if (!invalid_symbol_ptr) {
      ...
      if (!is_constant(parser->symbol)) {
        symbol_type = "variable / label symbol";
        if (!contains(symbol_table, parser->symbol)) {
          if (!add_entry(symbol_table, parser->symbol,
                         symbol_table->next_var_address, &err->error_code)) {
            ...
            case DUPLICATE_SYMBOL:
              printf("%s[ERROR] Duplicate variable symbol%s\n",
                     get_color_for_fd(fileno(stderr), RED),
                     get_color_for_fd(fileno(stderr), RESET));
              break;
            default:
      ...
      DEBUG_LOG(dbg,
                "found %s constant \"%s\" from the "
                "A-instruction",
                symbol_type, a_symbol);
      return;
    }
    _parser_symbol_print_error(ln, invalid_symbol_ptr, parser->typeString,
                               instruction, a_symbol, 1);
    parser->errorStatus = true;
    ...
    if (!closing_paren) {
      print_syntax_error(instruction, parser->typeString, ln,
                         (int)strlen(instruction), "missing ')'");
      parser->errorStatus = true;
    }
    ...
    if (!invalid_symbol_ptr) {
      ...
      DEBUG_LOG(dbg,
                "found label symbol \"%s\" from the L-instruction at line %d",
                l_symbol, ln);
      return;
    }
    _parser_symbol_print_error(ln, invalid_symbol_ptr, parser->typeString,
                               instruction, l_symbol, 1);
    parser->errorStatus = true;
  }
}
...
void parse_c_instruction(Parser *parser, TranslatedCode *code) {
  const char *invalid_instr_ptr =
      is_not_valid_c_instruction(parser->currentInstruction);
  ...
  int invalid_instr_pos =
      0 + (int)(strchr(parser->currentInstruction, *invalid_instr_ptr) -
  ...
  if (isspace((unsigned char)*invalid_instr_ptr)) {
    print_syntax_error(parser->currentInstruction, parser->typeString,
                       parser->lineNumber, invalid_instr_pos,
                       "invalid whitespace \'%c\'", *invalid_instr_ptr);
  } else {
    print_syntax_error(parser->currentInstruction, "NO-instruction",
                       parser->lineNumber, invalid_instr_pos,
